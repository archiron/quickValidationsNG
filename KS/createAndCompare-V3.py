...
for valGeV in listGeV: # loop over GUI configurations
    ...
    for dts in datasets:
        ...
        print('for banch enumeration, we use the files :')
        ...
        for i in range(0, N_histos):#, N_histos-1 range(N_histos - 1, N_histos):  # 1 N_histos histo for debug
            ...
            histo_KSref = f_KSref['DQMData/Run 1/EgammaV/Run summary/' + tp_1 + '/' + branches[i]]
            s_KSref = gr.histogram_to_dataframe5(histo_KSref)['Content']
            ...
            for elem in sortedRels2:
                ...
                with up.open(pathDATA + input_rel_file, open_options={"minimal_ttree_metadata": True}) as f_rel:
                    ...
                    s_new = gr.histogram_to_dataframe5(histo_rel)['Content']
                    ...
                    diffMax0 = DB.diffMAXKS3b(s_KSref, s_new)
